...
class Updater {
        ...
        static string EmptyString = "TODO";
        static string thread_string = "";
        ...
        static void Generate (Type type, string assembly, string ns, ref int count)
        {
                ...
                try {
                        ...
                        directory = new DirectoryInfo (root_directory.FullName + Path.DirectorySeparatorChar + ns);
                }

                string filename = String.Format ("{0}{1}{2}.xml", directory.FullName, Path.DirectorySeparatorChar, type.Name);
...
//                      Console.Write (GetNodeName (n.Node) + " ");
        ...
        static XmlDocument Generate (Type type)
        {
                ...
                //Globalization.CultureInfo ci = asm_name.CultureInfo;
                ...
                // Thread-safety node
                //
                type_node.AppendChild (AddElement (document, "ThreadSafetyStatement", thread_string));
        ...
        {
                string param_type = ConvertCTSName (parameter.ParameterType.FullName);
                string text = String.Format ("{0}: an object of type '{1}'", EmptyString,  param_type);
        ...
        static XmlElement AddDocsReturnsNode (XmlDocument document, Type return_value)
        {
                ...
                //
                // Return now if find out that it returns void here.
                //
                if (return_type == "void")
                        return null;
                string text = String.Format ("{0}: an object of type '{1}'", EmptyString,  return_type);
        ...
        static void AddProperty (XmlElement members, PropertyInfo property)
		{
			...
			if (property.GetSetMethod() != null)
				parameters = property.GetSetMethod().GetParameters();
			else if (property.GetGetMethod() != null)
				parameters = property.GetGetMethod().GetParameters();
        ...
        static void AddEvent (XmlElement members, EventInfo ev)
        {
                ...
                member.AppendChild (AddElement (document, "ReturnValue", null));
